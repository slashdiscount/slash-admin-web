...
const FreeUsers: React.FC = () => {
    ...
    dispatch(updateCurrentPath("Free Users", "list"));
    ...
    const getUsers = async () => {
        try {
            console.log("config",config)
            searchResults = {};
            const users = await Axios.get(`${API_URL}/free-users`, config);
    ...
    const fetchSkipUsers = async (page: number) => {
        try {
            if(search && search !== '' && searchResults && Object.keys(searchResults).length > 0){
                ...
                let users = await Axios.post(`${API_URL}/search-free-user?search=${search}&page=${page}`,{},config)
                console.log("users: ", users);
                return users.data.data.users;
            } else {

                let skippedUsers = await Axios.get(`${API_URL}/free-users?page=${page}`, config);
    ...
    function searchUsers(): void {
        if (search == '') {
            ...
            dispatch(commonSearch(`${API_URL}/search-free-user?search=${search}`,{type : 'user'}));
    ...
    return (
        <Fragment>
            ...
            <div className="row">
                ...
                <div className="col-sm col-lg-8 col-6 text-right">
                    <Link to={`/create-free-user`}>
                ...
                <div className="col-xl-12 col-lg-12">
                    <div className="card shadow mb-4">
                        ...
                        <div className="card-body">
                            <FreeUsersList users={usersDetails} />
                        </div>
                    </div>
                </div>
            </div>
